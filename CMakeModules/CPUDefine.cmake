...
elseif( MSVC )
	...
	if( cc_dumpmachine_res EQUAL 0 )
		...
		message( WARNING "will use the (sometimes incorrect) CMAKE_SYSTEM_PROCESSOR (${cpu}) to determine ARCH_STRING" )
	endif()
endif()
...
message( STATUS "Current CPU is: ${cpu}" )
...
message( STATUS "Current OS is: ${os}" )

add_definitions( -DOS_STRING="${os}" )

# Set the propper define for x84 or x64
if( CMAKE_SIZEOF_VOID_P EQUAL 8 )
    add_definitions( -Didx64=1 )
else()
    add_definitions( -Did386=1 )
endif()

# Set the dll subfix for the current systems
add_definitions( -DDLL_EXT="${CMAKE_SHARED_LIBRARY_SUFFIX}" )
message(STATUS "Shared library extension: ${CMAKE_SHARED_LIBRARY_SUFFIX}")

if( MSVC )
	# for MSVC ARCH_STRING is set in code (in code/qcommon/q_platform.h)
	message( STATUS "Setting -DOS_STRING=\"${os}\" - NOT setting ARCH_STRING, because we're targeting MSVC (VisualC++)" )
	# make sure ${cpu} isn't (cant't be) used by CMake when building with MSVC
	unset( cpu )
else()
	add_definitions( -DARCH_STRING="${cpu}" )
	message( STATUS "Setting -DARCH_STRING=\"${cpu}\" -DOS_STRING=\"${os}\" " )
